...
VAL_SPLIT_RATIO=${VAL_SPLIT_RATIO:-0.05}
VAL_MAX_SAMPLES=${VAL_MAX_SAMPLES:-}
EVAL_STEPS=${EVAL_STEPS:-5000}
PER_DEVICE_EVAL_BS=${PER_DEVICE_EVAL_BS:-${PER_DEVICE_BS}}
...
if ! [[ "$MAX_CHUNK_SIZE" =~ ^[0-9]+$ ]] || [ "$MAX_CHUNK_SIZE" -lt 1 ]; then
    echo "ERROR: MAX_CHUNK_SIZE must be a positive integer, got: $MAX_CHUNK_SIZE"
    exit 1
fi
if ! [[ "$NUM_FRAMES" =~ ^[0-9]+$ ]] || [ "$NUM_FRAMES" -lt 1 ]; then
    echo "ERROR: NUM_FRAMES must be a positive integer, got: $NUM_FRAMES"
    exit 1
fi
EXPECTED_NUM_FRAMES=$((8 * MAX_CHUNK_SIZE + 1))
if [ "$NUM_FRAMES" -ne "$EXPECTED_NUM_FRAMES" ]; then
    echo "ERROR: For the current LIBERO sampler, NUM_FRAMES must be 8 * MAX_CHUNK_SIZE + 1."
    echo "       Got NUM_FRAMES=$NUM_FRAMES MAX_CHUNK_SIZE=$MAX_CHUNK_SIZE expected NUM_FRAMES=$EXPECTED_NUM_FRAMES."
    exit 1
fi
NUM_VIEWS=${NUM_VIEWS:-2}
DEEPSPEED_CFG=${DEEPSPEED_CFG:-zero2}
ACTION_HEAD_CONFIG=${ACTION_HEAD_CONFIG:-wan_flow_matching_action_tf_wan22_224}
TRANSFORM_CONFIG=${TRANSFORM_CONFIG:-dreamzero_cotrain_libero}
DECOUPLE_VIDEO_ACTION_NOISE=${DECOUPLE_VIDEO_ACTION_NOISE:-true}
DECOUPLE_INFERENCE_NOISE=${DECOUPLE_INFERENCE_NOISE:-false}
...
echo "Using NUM_GPUS=$NUM_GPUS PER_DEVICE_BS=$PER_DEVICE_BS GLOBAL_BATCH_SIZE=$GLOBAL_BATCH_SIZE MAX_STEPS=${MAX_STEPS:-30000} TRAIN_ARCHITECTURE=$TRAIN_ARCHITECTURE SAVE_LORA_ONLY=$SAVE_LORA_ONLY TUNE_DIFFUSION_MODEL=$TUNE_DIFFUSION_MODEL USE_GRADIENT_CHECKPOINTING=$USE_GRADIENT_CHECKPOINTING SAVE_STRATEGY=$SAVE_STRATEGY SAVE_STEPS=$SAVE_STEPS DO_EVAL=$DO_EVAL EVAL_STRATEGY=$EVAL_STRATEGY EVAL_STEPS=$EVAL_STEPS VAL_SPLIT_RATIO=$VAL_SPLIT_RATIO VAL_MAX_SAMPLES=${VAL_MAX_SAMPLES:-none} PER_DEVICE_EVAL_BS=$PER_DEVICE_EVAL_BS DATALOADER_NUM_WORKERS=$DATALOADER_NUM_WORKERS DATASET_SHARD_SAMPLING_RATE=$DATASET_SHARD_SAMPLING_RATE MAX_CHUNK_SIZE=$MAX_CHUNK_SIZE NUM_FRAMES=$NUM_FRAMES DEEPSPEED_CFG=$DEEPSPEED_CFG ACTION_HEAD_CONFIG=$ACTION_HEAD_CONFIG TRANSFORM_CONFIG=$TRANSFORM_CONFIG DECOUPLE_VIDEO_ACTION_NOISE=$DECOUPLE_VIDEO_ACTION_NOISE DECOUPLE_INFERENCE_NOISE=$DECOUPLE_INFERENCE_NOISE IMAGE_RESOLUTION=${IMAGE_RESOLUTION_HEIGHT}x${IMAGE_RESOLUTION_WIDTH} FRAME_CACHE_ROOT=${FRAME_CACHE_ROOT:-none}"
...
"$PYTHON_BIN" -m torch.distributed.run --nproc_per_node "$NUM_GPUS" --standalone "$EXPERIMENT_PY" \
    ...
    action_head_cfg.config.decouple_video_action_noise=${DECOUPLE_VIDEO_ACTION_NOISE} \
    action_head_cfg.config.decouple_inference_noise=${DECOUPLE_INFERENCE_NOISE} \
    ...
    validation_max_samples=${VAL_MAX_SAMPLES:-null} \
    weight_decay=1e-5 \
    save_total_limit=10 \
